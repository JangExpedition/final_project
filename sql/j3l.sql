...
-- 액션
insert into movie values(1, '범죄도시1', '액션', '강윤성', '마동석, 윤계상', '한국의 불법 주정물 거래와 마약 카르텔 간의 대치를 그린 범죄 액션 영화', 15);
insert into movie values(2, '범죄도시2', '액션', '이상용', '마동석, 손석구', '한국의 대형 조직들이 서로 쟁탈하고 있는 대한민국 최대 광역단체인 신성파의 본거지를 둘러싸고 벌어지는 무력 충돌을 그리는 범죄 액션 영화', 15);
-- 코미디
insert into movie values(3, '극한직업', '코미디', '강윤성', '류승룡, 이하나', '대한민국 최고의 마약반 요원들이 범죄조직을 잡기 위해 위장 치킨집을 만들어 일어나는 일들을 보여주는 영화', 15);
insert into movie values(4, '행오버', '코미디', '토드 필립스', '브래들리 쿠퍼, 켄 정', '라스베이거스에서 벌어지는 4명의 친구들의 파티 이야기를 그린 코미디 영화', 15);
-- 로맨스
insert into movie values(5, '노트북', '로맨스', '닉 카사베티스', '라이언 고슬링, 레이첼 맥아담스', '니콜라스 스파크스의 소설을 원작으로 한 이 영화는, 두 사람의 첫사랑과 그 후 재회를 그리는 감동적인 이야기의 영화', 15);
insert into movie values(6, '어바웃 타임', '로맨스', '리처드 커티스', '돔놀 글리슨, 레이첼 맥아담스', '시간을 되돌릴 수 있는 능력을 가진 남자와 그의 연인 사이의 로맨스를 그린 영화', 12);
-- 애니메이션;
insert into movie values(7, '슬램덩크', '애니', '이노우에 다케히코', '키무라 스바루', '전국 제패를 꿈꾸는 북산고 농구부 5인방의 꿈과 열정, 멈추지 않는 도전을 그린 영화', 12);
insert into movie values(8, '너의 이름은', '애니', '신카이 마코토', '카미키 류노스케', '하나의 소년과 소녀가 미스터리한 방법으로 서로의 몸과 삶을 교환하게 되면서 시작되는 내용을 그린 영화', 12);
-- 호러
insert into movie values(9, '곡성', '호러', '나홍진', '황정민, 곽도원', '곡성의 한 마을에서 갑작스럽게 이상 사건이 연이어 일어나게 되면서 벌어지는 공포스러운 이야기를 그린 영화', 15);
insert into movie values(10, '겟 아웃', '호러', '조던 필', '다니엘 칼루야', '흑인 청년 크리스가 백인 여자친구 로즈의 가족을 만나러 갔을 때 벌어지는 일을 그린 영화', 18);
-- 다큐멘터리
insert into movie values(11, '멘 온 와이어', '다큐', '제임스 마쉬', '필립 프티', '1974년 8월 7일, 프랑스의 노트르담 대성당과 에펠탑 사이의 거리 200피트(61m) 높이에서 와이어를 건너는 것을 목표로 한 프랑스 출신 와이어 워커 필립 페티의 이야기를 다룬 다큐멘터리 영화', 12);
insert into movie values(12, '워낭소리', '다큐', '이충렬', '최원균', '소로 농사를 짓는 시골 할아버지와 소의 이야기를 그리는 다큐멘터리 독립 영화', 12);
-- 드라마
insert into movie values(13, '행복을 찾아서', '드라마', '가브리엘 무치노', '윌 스미스, 제이든 스미스', '실존 인물인 크리스 가드너의 삶의 여정을 따라가며, 그의 어려움과 자존심 상실, 가족과의 갈등 등을 그린 영화', 12);
insert into movie values(14, '블라인드 사이드', '드라마', '존 리 핸콕', '산드라 블록', '대학 입시에 실패한 마이클 오어가, 좌절과 무기력함 속에서 우연히 만난 부유한 백인 가정의 가장인 리얼리에게 입양되며 벌어지게 된 이야기를 다룬 영화', 12);
